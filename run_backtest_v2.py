"""V2 entry點:載入歷史K線,跑sweep反手backtest,印出表現摘要。"""
import datetime

import config
from data_loader import load_klines, klines_to_df
from backtest_engine_v2 import run_backtest_v2
...
def _report(result):
    ...
    print(f"=== {config.SYMBOL} v2(4H bias + 15m sweep)backtest "
...
def main():
    ...
    result = run_backtest_v2(df_1h, df_15m)
    _report(result)
